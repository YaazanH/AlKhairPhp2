...
<body>
<htmlpageheader name="pdf-header"><div class="pdf-header"><table class="header-table" dir="ltr"><tr><td class="header-side"><div class="date-box"><table class="date-table" dir="{{ app()->isLocale('ar') ? 'rtl' : 'ltr' }}"><tr><td class="date-label">{{ app()->isLocale('ar') ? 'من' : 'From' }}</td><td class="date-spacer">&nbsp;</td><td class="date-value" dir="ltr">{{ \Carbon\Carbon::parse($validated['date_from'])->format('d-m-Y') }}</td></tr><tr><td class="date-label">{{ app()->isLocale('ar') ? 'إلى' : 'To' }}</td><td class="date-spacer">&nbsp;</td><td class="date-value" dir="ltr">{{ \Carbon\Carbon::parse($validated['date_to'])->format('d-m-Y') }}</td></tr></table></div></td><td class="header-copy" dir="{{ app()->isLocale('ar') ? 'rtl' : 'ltr' }}"><div class="header-title">{{ __('workflow.teacher_attendance.export.report_title') }}</div><div class="header-subtitle">{{ $course?->name }}</div></td><td class="header-side" dir="rtl">@if($logo)<img src="{{ $logo }}" alt="">@endif</td></tr></table></div></htmlpageheader>
