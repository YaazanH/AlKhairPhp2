...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ config('app.supported_locales.'.app()->getLocale().'.direction', 'ltr') }}" class="dark">
    <head>
        ...
        <style>
            ...
            .report-card-preview-toolbar { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 20px; }
            ...
            @media print { body { background: white; padding: 0; } .report-card-preview-toolbar { display: none !important; } .print-template-sheet { box-shadow: none; margin: 0; } }
        </style>
    </head>
    <body>
        <div class="report-card-preview-toolbar">
            <h1 class="font-display text-4xl text-white">{{ __('print_templates.print.preview.title') }}</h1>
            <div class="admin-action-cluster">
                <button type="button" class="pill-link pill-link--accent" onclick="window.print()">{{ $printButtonLabel ?? __('print_templates.print.preview.buttons.print') }}</button>
                <a href="{{ $backUrl }}" class="pill-link">{{ $backButtonLabel }}</a>
            </div>
        </div>
