...
<div class="page-stack settings-admin-page">
    ...
    <section id="finance-legacy" class="grid gap-6 xl:grid-cols-2">
        ...
        <div class="surface-table">
            <div class="admin-grid-meta">
                <div><div class="admin-grid-meta__title">{{ __('settings.finance.sections.expense_category.table') }}</div></div>
                @can('finance.categories.manage')
            ...
            @error('expenseCategoryDelete') <div class="mx-5 mb-4 rounded-2xl border border-red-500/25 bg-red-500/10 px-3 py-2 text-sm text-red-200">{{ $message }}</div> @enderror
    ...
    <x-admin.modal :show="$showExpenseCategoryModal" :title="$expense_category_editing_id ? __('settings.finance.sections.expense_category.edit') : __('settings.finance.sections.expense_category.create')" :description="__('settings.finance.sections.expense_category.copy')" close-method="closeExpenseCategoryModal" max-width="3xl">
        <form wire:submit="saveExpenseCategory" class="space-y-4">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" wire:click="closeExpenseCategoryModal" class="pill-link">{{ __('finance.actions.cancel') }}</button>
