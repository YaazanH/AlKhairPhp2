...
new class extends Component {
    ...
    public function with(): array
    {
        ...
        return [
            'roles' => RoleRegistry::sortCollection(Role::query()
...
<div class="page-stack">
    ...
    @php
        $systemRolesCount = $roles->filter(fn ($role) => RoleRegistry::isSystemRole($role->name))->count();
        $customRolesCount = $roles->count() - $systemRolesCount;
    @endphp

    <section class="admin-kpi-grid">
        <article class="stat-card">
            <div class="kpi-label">{{ __('access.roles.stats.total') }}</div>
            <div class="metric-value mt-3">{{ number_format($roles->count()) }}</div>
        </article>
        <article class="stat-card">
        ...
        <section class="surface-table">
            <div class="admin-grid-meta">
                <div>
                    ...
                    <div class="admin-grid-meta__summary">{{ number_format($roles->count()) }} {{ __('crud.common.badges.in_view', ['count' => number_format($roles->count())]) }}</div>
                </div>
            </div>
                    ...
                    <table class="text-sm">
                        ...
                        <tbody class="divide-y divide-white/6">
                            @foreach ($roles as $role)
                                ...
                                @endphp
                                <tr class="{{ $isSelected ? 'bg-white/6' : '' }}">
                                    ...
                                    <td class="px-5 py-4 lg:px-6">
                                        <div class="flex flex-wrap justify-end gap-2">
                                            <button type="button" wire:click="selectRole('{{ $role->name }}')" class="pill-link pill-link--compact">
                                                {{ __('access.roles.actions.select') }}
                                            </button>
        ...
        <section class="surface-panel p-5 lg:p-6">
            @if ($selectedRoleRecord)
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <div class="eyebrow">{{ __('access.roles.editor.title') }}</div>
                        <h2 class="font-display mt-3 text-2xl text-white"><x-admin.role-label :name="$selectedRoleRecord->name" /></h2>
                ...
                <div class="mt-5 admin-filter-field">
