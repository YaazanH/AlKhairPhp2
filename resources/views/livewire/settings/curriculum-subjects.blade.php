...
<div class="page-stack settings-admin-page">
        ...
        @forelse($subjects as $subject)
            <div class="curriculum-subject-table overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-200 px-5 py-4 dark:border-neutral-700">
                    ...
                    <div class="flex gap-2"><button type="button" wire:click="editResource({{ $subject->id }})" class="pill-link pill-link--compact text-lg" aria-label="{{ __('curricula.actions.add_resource') }}">+</button><button type="button" wire:click="editSubject({{ $subject->id }})" class="pill-link pill-link--compact" aria-label="{{ __('curricula.actions.edit') }}">✎</button><button type="button" wire:click="deleteSubject({{ $subject->id }})" wire:confirm="{{ __('crud.common.confirm_delete.message') }}" class="pill-link pill-link--compact text-red-200" aria-label="{{ __('curricula.actions.delete') }}"><x-icons.trash /></button></div>
                </div>
                <div class="overflow-x-auto"><table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700"><thead class="bg-neutral-50 dark:bg-neutral-900/60"><tr><th data-curriculum-resource-index class="w-14 px-3 py-3 text-center font-medium">#</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.book_name') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.author') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.publisher') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.edition_number') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.published_on') }}</th><th class="px-5 py-3 text-right font-medium">{{ __('crud.common.actions.actions') }}</th></tr></thead><tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">@forelse($subject->resources as $resource)<tr><td class="w-14 px-3 py-3 text-center">{{ $loop->iteration }}</td><td class="px-5 py-3 text-white">{{ $resource->book_name }}</td><td class="px-5 py-3">{{ $resource->author ?: '—' }}</td><td class="px-5 py-3">{{ $resource->publisher ?: '—' }}</td><td class="px-5 py-3">{{ $this->displayEdition($resource->edition_number) }}</td><td class="px-5 py-3">{{ $resource->published_on?->format('Y') ?: '—' }}</td><td class="px-5 py-3"><div class="flex flex-nowrap justify-end gap-2"><button wire:click="editResource({{ $subject->id }}, {{ $resource->id }})" class="rounded-lg border border-neutral-300 px-3 py-1.5 dark:border-neutral-700" aria-label="{{ __('curricula.actions.edit') }}">{{ __('crud.common.actions.edit') }}</button><button wire:click="deleteResource({{ $resource->id }})" wire:confirm="{{ __('crud.common.confirm_delete.message') }}" class="rounded-lg border border-red-300 px-3 py-1.5 text-red-700 dark:border-red-800 dark:text-red-300" aria-label="{{ __('curricula.actions.delete') }}">{{ __('crud.common.actions.delete') }}</button></div></td></tr>@empty<tr><td colspan="7" class="admin-empty-state">{{ __('crud.common.not_available') }}</td></tr>@endforelse</tbody></table></div>
            </div>
    ...
    <x-admin.modal :show="$showStandaloneResourcesModal" :title="__('curricula.fields.standalone_books')" close-method="closeStandaloneResources" max-width="5xl">
        ...
        <div class="space-y-4">
            ...
            @endif
            <div class="curriculum-resource-table overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700"><div class="overflow-x-auto"><table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700"><thead class="bg-neutral-50 dark:bg-neutral-900/60"><tr><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.book_name') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.edition_number') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.published_on') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.pdf_file_short') }}</th><th class="px-5 py-3 text-right font-medium">{{ __('crud.common.actions.actions') }}</th></tr></thead><tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">@forelse($standaloneResources as $resource)<tr><td class="px-5 py-3 text-white">{{ $resource->book_name }}</td><td class="px-5 py-3">{{ $this->displayEdition($resource->edition_number) }}</td><td class="px-5 py-3">{{ $resource->published_on?->format('Y') ?: '—' }}</td><td class="px-5 py-3">{{ $resource->pdf_path ? __('curricula.fields.pdf_uploaded') : __('curricula.fields.no_pdf') }}</td><td class="px-5 py-3"><div class="flex flex-nowrap justify-end gap-2"><button wire:click="editStandaloneResource({{ $resource->id }})" class="rounded-lg border border-neutral-300 px-3 py-1.5 dark:border-neutral-700" aria-label="{{ __('curricula.actions.edit') }}">{{ __('crud.common.actions.edit') }}</button><button wire:click="deleteResource({{ $resource->id }})" wire:confirm="{{ __('crud.common.confirm_delete.message') }}" class="rounded-lg border border-red-300 px-3 py-1.5 text-red-700 dark:border-red-800 dark:text-red-300" aria-label="{{ __('curricula.actions.delete') }}">{{ __('crud.common.actions.delete') }}</button></div></td></tr>@empty<tr><td colspan="5" class="admin-empty-state">{{ __('crud.common.not_available') }}</td></tr>@endforelse</tbody></table></div></div>
        </div>
    </x-admin.modal>
