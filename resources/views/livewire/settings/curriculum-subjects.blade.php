    ...
    <section class="grid gap-4">
        @forelse($subjects as $subject)
            <div class="curriculum-subject-table overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
                ...
                <div class="overflow-x-auto"><table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700"><thead class="bg-neutral-50 dark:bg-neutral-900/60"><tr><th data-curriculum-resource-index class="curriculum-resource-index w-14 px-3 py-3 text-center font-medium">#</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.book_name') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.author') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.publisher') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.edition_number') }}</th><th class="px-5 py-3 text-left font-medium">{{ __('curricula.fields.published_on') }}</th><th class="px-5 py-3 text-right font-medium">{{ __('crud.common.actions.actions') }}</th></tr></thead><tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">@forelse($subject->resources as $resource)<tr><td class="curriculum-resource-index w-14 px-3 py-3 text-center">{{ $loop->iteration }}</td><td class="px-5 py-3 text-white">{{ $resource->book_name }}</td><td class="px-5 py-3">{{ $resource->author ?: '—' }}</td><td class="px-5 py-3">{{ $resource->publisher ?: '—' }}</td><td class="px-5 py-3">{{ $this->displayEdition($resource->edition_number) }}</td><td class="px-5 py-3">{{ $resource->published_on?->format('Y') ?: '—' }}</td><td class="px-5 py-3"><div class="flex flex-nowrap justify-end gap-2"><button wire:click="editResource({{ $subject->id }}, {{ $resource->id }})" class="rounded-lg border border-neutral-300 px-3 py-1.5 dark:border-neutral-700" aria-label="{{ __('curricula.actions.edit') }}">{{ __('crud.common.actions.edit') }}</button></div></td></tr>@empty<tr><td colspan="7" class="admin-empty-state">{{ __('crud.common.not_available') }}</td></tr>@endforelse</tbody></table></div>
            </div>
