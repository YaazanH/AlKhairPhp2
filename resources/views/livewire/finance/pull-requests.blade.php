...
new class extends Component {
    ...
    public function submitRequest(): void
    {
        ...
        $this->reset(['requested_amount', 'activity_id', 'cash_box_id', 'requested_reason', 'accepted_terms']);
        session()->flash('status', $canReview ? 'Pull request posted.' : 'Pull request sent to finance management.');
    }
    ...
    public function accept(int $requestId): void
    {
        ...
        session()->flash('status', 'Pull request accepted and posted.');
    }
    ...
    public function decline(int $requestId): void
    {
        ...
        session()->flash('status', 'Pull request declined.');
    }
}; ?>
...
<div class="page-stack">
    <section class="page-hero p-6 lg:p-8">
        ...
        <h1 class="font-display mt-4 text-4xl leading-none text-white md:text-5xl">Pull Requests</h1>
        <p class="mt-4 max-w-3xl text-base leading-7 text-neutral-200">Teachers request local-currency withdrawals. Finance management can accept a different amount and choose the paying cash box.</p>
    </section>
    ...
    @can('finance.pull-requests.create')
        <section class="surface-panel p-5 lg:p-6">
            <div class="admin-section-card__title">New pull request</div>
            <form wire:submit="submitRequest" class="mt-5 grid gap-4 lg:grid-cols-4">
                <div><label class="mb-1 block text-sm font-medium">Amount ({{ $localCurrency->code }})</label><input wire:model="requested_amount" type="number" min="0" step="0.01" class="w-full rounded-xl px-4 py-3 text-sm">@error('requested_amount') <div class="mt-1 text-sm text-red-400">{{ $message }}</div> @enderror</div>
                <div><label class="mb-1 block text-sm font-medium">Activity</label><select wire:model="activity_id" class="w-full rounded-xl px-4 py-3 text-sm"><option value="">No activity</option>@foreach ($activities as $activity)<option value="{{ $activity->id }}">{{ $activity->title }} | {{ $activity->activity_date?->format('Y-m-d') }}</option>@endforeach</select></div>
                @can('finance.pull-requests.review')
                    <div><label class="mb-1 block text-sm font-medium">Cash box</label><select wire:model="cash_box_id" class="w-full rounded-xl px-4 py-3 text-sm"><option value="">Choose box</option>@foreach ($cashBoxes as $box)<option value="{{ $box->id }}">{{ $box->name }}</option>@endforeach</select>@error('cash_box_id') <div class="mt-1 text-sm text-red-400">{{ $message }}</div> @enderror</div>
                @endcan
                <div class="lg:col-span-4"><label class="mb-1 block text-sm font-medium">Reason</label><textarea wire:model="requested_reason" rows="2" class="w-full rounded-xl px-4 py-3 text-sm"></textarea></div>
                @if ($terms !== '')
                ...
                @endif
                <div class="lg:col-span-4"><button type="submit" class="pill-link pill-link--accent">Submit request</button></div>
            </form>
        </section>
    @endcan

    <section class="surface-table">
        <div class="admin-grid-meta"><div><div class="admin-grid-meta__title">Pull request ledger</div><div class="admin-grid-meta__summary">{{ __('crud.common.badges.in_view', ['count' => number_format($requests->total())]) }}</div></div></div>
        <div class="overflow-x-auto">
            <table class="text-sm">
                <thead><tr><th class="px-5 py-3 text-left">Request</th><th class="px-5 py-3 text-left">Requester</th><th class="px-5 py-3 text-left">Activity</th><th class="px-5 py-3 text-left">Amounts</th><th class="px-5 py-3 text-left">Status</th><th class="px-5 py-3 text-right">Review</th></tr></thead>
                <tbody class="divide-y divide-white/6">
                    @forelse ($requests as $request)
                        <tr>
                            ...
                            <td class="px-5 py-3">{{ $request->requestedBy?->name ?: '-' }}</td>
                            <td class="px-5 py-3">{{ $request->activity?->title ?: '-' }}</td>
                            <td class="px-5 py-3"><div>Requested: {{ number_format((float) $request->requested_amount, 2) }} {{ $request->requestedCurrency?->code }}</div><div class="text-xs text-neutral-500">Accepted: {{ $request->accepted_amount !== null ? number_format((float) $request->accepted_amount, 2).' '.$request->acceptedCurrency?->code : '-' }}</div></td>
                                ...
                                <div class="admin-action-cluster admin-action-cluster--end">
                                    ...
                                    @can('finance.pull-requests.review')
                                        @if ($request->status === 'pending')
                                            ...
                                            <select wire:model="review_cash_boxes.{{ $request->id }}" class="w-36 rounded-xl px-3 py-2 text-sm"><option value="">Box</option>@foreach ($cashBoxes as $box)<option value="{{ $box->id }}">{{ $box->name }}</option>@endforeach</select>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-5 py-10 text-center text-sm text-neutral-500">No pull requests yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
