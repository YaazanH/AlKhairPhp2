...
<div class="page-stack">
    ...
    <section class="admin-kpi-grid">
        <article class="stat-card"><div class="kpi-label">{{ __('finance.fields.income') }}</div><div class="metric-value mt-3">{{ number_format($report['summary']['income'], 2) }}</div></article>
        <article class="stat-card"><div class="kpi-label">{{ __('finance.fields.expense') }}</div><div class="metric-value mt-3">{{ number_format($report['summary']['expense'], 2) }}</div></article>
        <article class="stat-card"><div class="kpi-label">{{ __('finance.fields.net') }}</div><div class="metric-value mt-3">{{ number_format($report['summary']['net'], 2) }}</div></article>
    ...
    <section class="grid gap-6 xl:grid-cols-2">
        <div class="surface-table">
            ...
            <div class="overflow-x-auto"><table class="text-sm"><thead><tr><th class="px-5 py-3 text-left">{{ __('finance.common.currency') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.income') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.expense') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.net') }}</th></tr></thead><tbody class="divide-y divide-white/6">@forelse ($report['summary_by_currency'] as $row)<tr><td class="px-5 py-3">{{ $row['currency']?->code }}</td><td class="px-5 py-3">{{ number_format($row['income'], 2) }}</td><td class="px-5 py-3">{{ number_format($row['expense'], 2) }}</td><td class="px-5 py-3">{{ number_format($row['net'], 2) }}</td></tr>@empty<tr><td colspan="4" class="px-5 py-10 text-center text-sm text-neutral-500">{{ __('finance.empty.no_transactions') }}</td></tr>@endforelse</tbody></table></div>
        </div>
        ...
        <div class="surface-table">
            ...
            <div class="overflow-x-auto"><table class="text-sm"><thead><tr><th class="px-5 py-3 text-left">{{ __('finance.fields.cash_box') }}</th><th class="px-5 py-3 text-left">{{ __('finance.common.currency') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.balance') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.local_equivalent') }}</th></tr></thead><tbody class="divide-y divide-white/6">@foreach ($report['balances'] as $boxBalance)@foreach ($boxBalance['currencies'] as $row)<tr><td class="px-5 py-3">{{ $boxBalance['cash_box']->name }}</td><td class="px-5 py-3">{{ $row['currency']->code }}</td><td class="px-5 py-3">{{ number_format($row['balance'], 2) }}</td><td class="px-5 py-3">{{ number_format($row['local_equivalent'], 2) }}</td></tr>@endforeach@endforeach</tbody></table></div>
        </div>

        <div class="surface-table">
            <div class="admin-grid-meta"><div><div class="admin-grid-meta__title">{{ __('finance.reports.quarter_totals') }}</div></div></div>
            <div class="overflow-x-auto"><table class="text-sm"><thead><tr><th class="px-5 py-3 text-left">{{ __('finance.fields.quarter') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.period') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.net_local') }}</th></tr></thead><tbody class="divide-y divide-white/6">@foreach ($report['quarter_totals'] as $quarter)<tr><td class="px-5 py-3">Q{{ $quarter['quarter'] }}</td><td class="px-5 py-3">{{ $quarter['start']->format('Y-m-d') }} - {{ $quarter['end']->format('Y-m-d') }}</td><td class="px-5 py-3">{{ number_format($quarter['net'], 2) }}</td></tr>@endforeach</tbody></table></div>
        </div>
    </section>
    ...
    <section class="surface-table">
        ...
        <div class="overflow-x-auto"><table class="text-sm"><thead><tr><th class="px-5 py-3 text-left">{{ __('finance.fields.category') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.income') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.expense') }}</th><th class="px-5 py-3 text-left">{{ __('finance.fields.net') }}</th></tr></thead><tbody class="divide-y divide-white/6">@foreach ($report['category_totals'] as $row)<tr><td class="px-5 py-3">{{ $row['category'] }}</td><td class="px-5 py-3">{{ number_format($row['income'], 2) }}</td><td class="px-5 py-3">{{ number_format($row['expense'], 2) }}</td><td class="px-5 py-3">{{ number_format($row['net'], 2) }}</td></tr>@endforeach</tbody></table></div>
    </section>
