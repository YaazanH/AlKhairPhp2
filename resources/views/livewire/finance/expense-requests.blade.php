...
new class extends Component {
    ...
    public function submitRequest(): void
    {
        ...
        if (! $validated['activity_id'] && ! $validated['teacher_id'] && ! $validated['finance_category_id']) {
            $this->addError('finance_category_id', 'Select an activity, teacher, or management category.');
        ...
        $this->currency_id = app(FinanceService::class)->localCurrency()->id;
        session()->flash('status', $canReview ? 'Expense posted.' : 'Expense request sent.');
    }
    ...
    public function accept(int $requestId): void
    {
        ...
        session()->flash('status', 'Expense request accepted and posted.');
    }
    ...
    public function decline(int $requestId): void
    {
        ...
        session()->flash('status', 'Expense request declined.');
    }
...
<div class="page-stack">
    <section class="page-hero p-6 lg:p-8">
        ...
        <h1 class="font-display mt-4 text-4xl leading-none text-white md:text-5xl">Expense Requests</h1>
        <p class="mt-4 max-w-3xl text-base leading-7 text-neutral-200">Record expenses for activities, teachers, or management categories. Finance management entries post immediately.</p>
    </section>
    ...
    @can('finance.expense-requests.create')
        <section class="surface-panel p-5 lg:p-6">
            <div class="admin-section-card__title">New expense</div>
            <form wire:submit="submitRequest" class="mt-5 grid gap-4 lg:grid-cols-3">
                ...
                @can('finance.expense-requests.review')<div><label class="mb-1 block text-sm font-medium">Cash box</label><select wire:model="cash_box_id" class="w-full rounded-xl px-4 py-3 text-sm"><option value="">Choose box</option>@foreach ($cashBoxes as $box)<option value="{{ $box->id }}">{{ $box->name }}</option>@endforeach</select>@error('cash_box_id') <div class="mt-1 text-sm text-red-400">{{ $message }}</div> @enderror</div>@endcan
                <div><label class="mb-1 block text-sm font-medium">Activity</label><select wire:model="activity_id" class="w-full rounded-xl px-4 py-3 text-sm"><option value="">No activity</option>@foreach ($activities as $activity)<option value="{{ $activity->id }}">{{ $activity->title }}</option>@endforeach</select></div>
                ...
                <div><label class="mb-1 block text-sm font-medium">Category</label><select wire:model="finance_category_id" class="w-full rounded-xl px-4 py-3 text-sm"><option value="">Choose category</option>@foreach ($categories as $category)<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach</select>@error('finance_category_id') <div class="mt-1 text-sm text-red-400">{{ $message }}</div> @enderror</div>
                ...
                <div class="lg:col-span-3"><label class="mb-1 block text-sm font-medium">Invoices / attachments</label><input wire:model="attachments" type="file" multiple class="w-full rounded-xl px-4 py-3 text-sm">@error('attachments.*') <div class="mt-1 text-sm text-red-400">{{ $message }}</div> @enderror</div>
                <div class="lg:col-span-3"><button type="submit" class="pill-link pill-link--accent">Save expense</button></div>
            </form>
        </section>
    @endcan

    @include('livewire.finance.partials.requests-table', ['requests' => $requests, 'cashBoxes' => $cashBoxes, 'reviewPermission' => 'finance.expense-requests.review'])
</div>
