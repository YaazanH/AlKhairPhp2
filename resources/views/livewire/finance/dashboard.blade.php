...
@php
    ...
    $pieOffset = 0.0;
    $quarterExpensePeak = max(0.0, (float) collect($report['quarter_totals'])->concat($report['previous_year_quarter_totals'])->max('expense'));
    if ($quarterExpensePeak > 0) {
        $quarterRoughStep = $quarterExpensePeak / 6;
        $quarterMagnitude = 10 ** floor(log10($quarterRoughStep));
        $quarterNormalizedStep = $quarterRoughStep / $quarterMagnitude;
        $quarterStepFactor = match (true) {
            $quarterNormalizedStep <= 1 => 1,
            $quarterNormalizedStep <= 2 => 2,
            $quarterNormalizedStep <= 2.5 => 2.5,
            $quarterNormalizedStep <= 5 => 5,
            default => 10,
        };
        $quarterTickStep = $quarterStepFactor * $quarterMagnitude;
        $quarterExpenseMax = ceil($quarterExpensePeak / $quarterTickStep) * $quarterTickStep;
    } else {
        $quarterTickStep = 1.0;
        $quarterExpenseMax = 5.0;
    }
    $quarterTickValues = collect(range(0, (int) round($quarterExpenseMax / $quarterTickStep)))
        ->map(fn (int $index): float => $index * $quarterTickStep);
    $quarterTickDecimals = $quarterTickStep < 1 ? 2 : 0;
    $quarterX = fn (int $index) => 82 + ($index * 116);
...
<div class="page-stack finance-dashboard" data-finance-dashboard>
    ...
    <x-admin.modal :show="$showTransferModal" :title="__('finance.dashboard.move_money')" close-method="$set('showTransferModal', false)" max-width="3xl">
        <form wire:submit="transfer" class="grid gap-4 md:grid-cols-2">
            <div><label class="mb-1 block text-sm">{{ __('finance.fields.from') }}</label><select wire:model.live="transfer_from_cash_box_id" class="w-full rounded-xl px-4 py-3">@foreach ($cashBoxes as $fund)<option value="{{ $fund->id }}">{{ $fund->name }}</option>@endforeach</select></div>
            <div><label class="mb-1 block text-sm">{{ __('finance.fields.to') }}</label><select wire:model="transfer_to_cash_box_id" class="w-full rounded-xl px-4 py-3"><option value="">-</option>@foreach ($transferToCashBoxes as $fund)<option value="{{ $fund->id }}">{{ $fund->name }}</option>@endforeach</select></div>
            <div class="md:col-span-2">
                <label class="mb-1 block text-sm">{{ __('finance.fields.amount') }}</label>
                <x-finance.amount-input amount-model="transfer_amount" currency-model="transfer_currency_id" :currencies="$transferCurrencies" />
                @error('transfer_amount')<div class="text-sm text-red-400">{{ $message }}</div>@enderror
            </div>
    ...
    <x-admin.modal :show="$showCreateRequestModal" :title="__('finance.pull_requests.new')" close-method="$set('showCreateRequestModal', false)" max-width="4xl">
        <form wire:submit="createRequest" class="grid gap-4 md:grid-cols-2">
            <div><label class="mb-1 block text-sm">{{ __('finance.fields.requester') }}</label><select wire:model="request_teacher_id" class="w-full rounded-xl px-4 py-3"><option value="">-</option>@foreach ($teachers as $teacher)<option value="{{ $teacher->id }}">{{ trim($teacher->first_name.' '.$teacher->last_name) }}</option>@endforeach</select></div>
            <div><label class="mb-1 block text-sm">{{ __('finance.fields.category') }}</label><select wire:model.live="request_kind_id" class="w-full rounded-xl px-4 py-3"><option value="">-</option>@foreach ($requestKinds as $kind)<option value="{{ $kind->id }}">{{ $kind->name }} - {{ __('finance.pull_modes.'.$kind->mode) }}</option>@endforeach</select></div>
            <div><label class="mb-1 block text-sm">{{ __('finance.fields.cash_box') }}</label><select wire:model="request_cash_box_id" class="w-full rounded-xl px-4 py-3"><option value="">-</option>@foreach ($requestCashBoxes as $fund)<option value="{{ $fund->id }}">{{ $fund->name }}</option>@endforeach</select></div>
            <div>
                <label class="mb-1 block text-sm">{{ __('finance.fields.amount') }}</label>
                <x-finance.amount-input amount-model="request_amount" currency-model="request_currency_id" :currencies="$currencies" />
                @error('request_amount')<div class="text-sm text-red-400">{{ $message }}</div>@enderror
            </div>
    ...
    <x-admin.modal :show="$showRequestHistoryModal" :title="__('finance.dashboard.previous_requests')" close-method="$set('showRequestHistoryModal', false)" max-width="6xl" compact>
        ...
        @if ($showRequestHistoryModal && $requestHistory->hasPages())
            ...
            <div class="overflow-x-auto">
                <table class="financial-transactions-table w-full text-sm">
                    <colgroup>
                        ...
                        <col style="width: 20.725%" data-finance-spacing-column="category">
                        <col style="width: 8.7125%" data-finance-reduced-column="debit" data-finance-left-column data-finance-left-weight="8.7125">
                        <col style="width: 8.7125%" data-finance-reduced-column="credit" data-finance-left-column data-finance-left-weight="8.7125">
                        <col style="width: 10.05%" data-finance-reduced-column="balance" data-finance-left-column data-finance-left-weight="10.05">
                        <col style="width: 10.05%" data-finance-left-column data-finance-left-weight="10.05">
                    </colgroup>
                    <thead><tr><th class="px-2 py-3 text-center">#</th><th class="px-3 py-3 text-left">{{ __('finance.common.date') }}</th><th class="px-3 py-3 text-left"><span class="finance-transaction-kashida-label" data-finance-kashida-label="reference" data-finance-kashida-target data-finance-spacing-content="reference">{{ __('finance.fields.transaction_no') }}</span></th><th class="px-3 py-3 text-left"><span class="finance-transaction-kashida-label" data-finance-kashida-label="fund" data-finance-kashida-target data-finance-spacing-content="fund">{{ __('finance.fields.cash_box') }}</span></th><th class="px-3 py-3 text-left"><span class="finance-transaction-kashida-label" data-finance-kashida-label="type" data-finance-spacing-content="type">{{ __('finance.fields.transaction_type') }}</span></th><th class="px-3 py-3 text-left" data-finance-spacing-content="category" data-finance-double-width>{{ __('finance.fields.category') }}</th><th class="px-3 py-3 text-left">{{ __('finance.fields.debit') }}</th><th class="px-3 py-3 text-left">{{ __('finance.fields.credit') }}</th><th class="px-3 py-3 text-left">{{ __('finance.fields.fund_balance') }}</th><th class="px-3 py-3 text-left">{{ __('finance.fields.user') }}</th></tr></thead>
                    <tbody class="divide-y divide-white/6">
                        @foreach ($transactions as $transaction)
                            ...
                            <tr class="{{ $transaction->trashed() ? 'opacity-50' : '' }}">
                                ...
                                <td class="px-3 py-3"><div data-finance-spacing-content="category"><div class="finance-transaction-primary">{{ app(FinanceService::class)->transactionCategoryLabel($transaction) }}</div>@if($transaction->description)<div class="finance-transaction-secondary mt-1 text-xs text-neutral-500">{{ $transaction->description }}</div>@endif</div></td>
