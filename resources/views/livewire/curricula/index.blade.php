...
<div class="page-stack">
    ...
    @if($isManager)
        ...
        <section class="surface-table" data-curricula-index-table><div class="admin-toolbar p-5" data-curricula-table-toolbar><div class="curricula-table-heading"><div class="admin-toolbar__title">{{ __('curricula.table.curricula') }}</div></div><button wire:click="openCurriculum" class="curriculum-add-action pill-link pill-link--accent" title="{{ __('curricula.actions.add_curriculum') }}" aria-label="{{ __('curricula.actions.add_curriculum') }}"><span>{{ __('curricula.actions.add_curriculum') }}</span><svg data-curricula-add-icon aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5.5v13" vector-effect="non-scaling-stroke" /><path d="M5.5 12h13" vector-effect="non-scaling-stroke" /></svg></button></div><div class="overflow-x-auto"><table class="w-full table-fixed text-sm"><thead><tr><th class="p-4">{{ __('curricula.fields.name') }}</th><th class="p-4">{{ __('curricula.fields.grade') }}</th><th class="p-4">{{ __('curricula.fields.subjects') }}</th><th class="p-4">{{ __('curricula.fields.lessons') }}</th><th class="p-4"></th></tr></thead><tbody>@forelse($curricula as $curriculum)<tr><td class="p-4 font-semibold text-white">{{ $curriculum->name }}</td><td class="p-4">{{ $curriculum->gradeLevel?->name ?: '—' }}</td><td class="p-4">{{ $curriculum->subjects->pluck('definition.name')->filter()->implode('، ') ?: '—' }}</td><td class="p-4">{{ $curriculum->subjects->sum(fn ($subject) => $subject->lessons->count()) }}</td><td class="p-4 text-end"><a href="{{ route('curricula.show', $curriculum) }}" wire:navigate class="pill-link pill-link--compact">{{ __('curricula.actions.open') }}</a></td></tr>@empty<tr><td colspan="5" class="admin-empty-state">{{ __('curricula.table.empty') }}</td></tr>@endforelse</tbody></table></div></section>
    @else
        @if($selectedGroup)
