...
class InvoiceOcrService
{
    ...
    private function prepareImage(string $path, int $page): array
    {
        ...
        $image = $this->loadImage($path, (int) $dimensions[2]);
        if (! $image) {
            throw new RuntimeException('invoice_capture.errors.unreadable');
        }
        try {
    ...
    private function run(array $command, float $deadline): string
    {
        ...
        if (! $process->isSuccessful()) {
            ...
            $lowerError = Str::lower($error);
            if (in_array($process->getExitCode(), [126, 127], true)
                || str_contains($error, 'Error opening data file')
                || str_contains($lowerError, 'not recognized')
                || str_contains($lowerError, 'no such file or directory')
                || str_contains($lowerError, 'the system cannot find')) {
                throw new RuntimeException('invoice_capture.errors.unavailable');
            }
            throw new RuntimeException('invoice_capture.errors.unreadable');
        }
